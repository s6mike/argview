...
[![](examples/2cc90b7c54f340b9e4589f2c1b5a26589a5f2653.png)](https://atlas.mindmup.com/2022/07/aae05340f9f311eca4149f3188cae501/example_mup/index.html)
...
This works on Debian, but has not been tested on any other platforms. I believe it should be portable, but I'm not familiar enough with Lua to know for sure.

Place `argmap2mup.lua`, `argmap2tikz.lua`, and `mup2argmap.lua` somewhere in your PATH e.g. ~/bin.
...
1. To install these dependencies, you can use the included rockspec file. e.g. when in the package directory, run
`luarocks make` or `luarocks install --only-deps argmap-3.1.2-3.rockspec` (install accesses rockspec from github repo instead of locally, there is probably no advantage in this).
...
4. Converting the `tikz` code generated by `argmap2tikz.lua` to PDF requires several `TeX` packages, some of which you may need to install if they are not already available on your system.

5. `pandoc-argmap.lua` depends on [`pandoc`](https://pandoc.org/), and on `argmap2lua` and `argmap2tikz.lua`. It also depends on
...
Note: since this readme was created, mindmup no longer seems to read all the mup files generated by argmap2mup (see [Issue #11](https://github.com/s6mike/argmap/issues/11)). However, [legacy mindmup](https://github.com/mindmup/mapjs-webpack-example) can still be used to read them.
...
`$ cat example.yml | lua argmap2mup.lua > example.mup`: generate a MindMup map from example.yml. Import that map into MindMup to work with.

`$ lua argmap2mup.lua -u example-2.yml`: generate a MindMup map and upload it to your Google Drive. You can then go to Google Drive and open the map in MindMup to work with it.
...
``` shell
lua mup2argmap.lua download.mup > download.yml
```

Next, try generating a PDF image from `example.yml`:

``` shell
$ cat example.yml | lua argmap2tikz.lua -s > example.tex
$ lualatex example
```
...
``` shell
$ cat example.yml | lua argmap2mup.lua | jq
{
  ⋮
  "ideas": {
    "1": {
...
``` shell
$  lua argmap2mup.lua -u example.yml
...
``` shell
$ cat example.mup | lua mup2argmap.lua
$ lua mup2argmap.lua example.mup
```
...
`argmap2tikz.lua` is a pipe for converting `YAML` maps to TikZ pictures, suitable for embedding in `LaTeX`.

It takes as its input the first CLI argument that is not an option, or, if there is no such argument, STDIN.

``` shell
$ cat example.yml | lua argmap2tikz.lua
\begin{tikzpicture}
  ⋮ 
  {
...
``` shell
$ lua argmap2tikz.lua -s example.yml
...
``` shell
$ lua argmap2tikz.lua -s example.yml > example.tex
$ lualatex example
...
``` shell
$ lua argmap2tikz.lua -i > header.tex
