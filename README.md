...
Note: since this readme was created, mindmup no longer seems to read all the mup files generated by `argmap2mup.lua` (see [Issue #11](https://github.com/s6mike/argmap/issues/11)). However, [legacy mindmup - mapjs](#displaying-argmaps-with-mapjs) can still be used to read them.

See [`scripts/bash_aliases_argmap.sh`](scripts/bash_aliases_argmap.sh) for bash commands to simplify execution. Note that some use the mindmup mapjs repo for displaying the mup file using their legacy open source code.
