...
The map above is also a hyper-link: click on it to open an version of the map in [Mindmup's Argument Visualization Mode](https://www.mindmup.com/tutorials/argument-visualization.html).
...
Place `pandoc-argmap.lua` in the `filters` folder inside your pandoc data directory, e.g. `$HOME/.pandoc/filters/pandoc-argmap.lua`.

### Dependencies

Lua: using Lua version 5.3.4, luarocks 3.7.0

`argmap2mup`, `argmap2tikz`, and `mup2argmap` require [penlight](https://github.com/stevedonovan/Penlight) (for command line option parsing), [lyaml](https://github.com/gvvaughan/lyaml) (for parsing `YAML`), and [json](https://github.com/rxi/json.lua) (for encoding `JSON`) and [LuaLogging: A simple API to use logging features in Lua](https://neopallium.github.io/lualogging/manual.html#introduction).

To install these dependencies, you can use the included rockspec file.

`luarocks install --only-deps argmap-2.2.0-1.rockspec`

For lyaml, you may need to install [yaml (conda)](https://anaconda.org/anaconda/yaml) or [libyaml-dev](https://packages.debian.org/stretch/libyaml-dev).

`argmap2mup` and `mup2argmap` also depend on the command line utility [`gdrive`](https://github.com/prasmussen/gdrive) for Google Drive integration. Follow the link for installation instructions. (Note that on linux the 386 version is more likely to work: see https://github.com/prasmussen/gdrive/issues/597).
...
`pandoc-argmap.lua` depends on [`pandoc`](https://pandoc.org/), and on `argmap2lua` and `argmap2tikz`. It also depends on
[`pdf2svg`](http://www.cityinthesky.co.uk/opensource/pdf2svg/) for conversion to svg, and [ImageMagick](https://www.imagemagick.org/)'s `convert` for conversion to png.
...
`$ cat example.yml | lua argmap2mup > example.mup`: generate a MindMup map from example.yml. Import that map into MindMup to work with.

`$ lua argmap2mup -u example-2.yml`: generate a MindMup map and upload it to your Google Drive. You can then go to Google Drive and open the map in MindMup to work with it.
...
``` shell
lua mup2argmap download.mup > download.yml
```

Next, try generating a PDF image from `example.yml`:

``` shell
$ cat example.yml | lua argmap2tikz -s > example.tex
$ lualatex example
```
...
`argmap2mup` is a pipe for converting `YAML` maps to `JSON` encoded MindMup maps.

It takes as its input the first CLI argument that is not an option, or, if there is no such argument, STDIN.

``` shell
$ cat example.yml | lua argmap2mup | jq
{
  ⋮
  "ideas": {
    "1": {
...
``` shell
$ cat example.yml | lua argmap2mup -g 1e4HAl1iHPKBiKZ_BI_yBw7rXYbuvMsC2
1e4HAl1iHPKBiKZ_BI_yBw7rXYbuvMsC2
```
...
``` shell
$ cat example.mup | lua mup2argmap
$ lua mup2argmap example.mup
```
...
`argmap2tikz` is a pipe for converting `YAML` maps to TikZ pictures, suitable for embedding in `LaTeX`.

It takes as its input the first CLI argument that is not an option, or, if there is no such argument, STDIN.

``` shell
$ cat example.yml | lua argmap2tikz
\begin{tikzpicture}
  ⋮ 
  {
...
``` shell
$ lua argmap2tikz -s example.yml > example.tex
$ lualatex example
...
filter](https://pandoc.org/lua-filters.html) for converting `YAML` argument maps embedded in a pandoc document, using `argmap2mup` and `argmap2tikz`.
...
For all other formats, it uploads the mindmup file to Google Drive and generates individual pdf files for each map, and then converts them to png
using ImageMagick's `convert` command. It replaces the code block with a paragraph containing the generated image, linked to the mindmup file.
...
So have updated above instructions to include lua commands where appropriate.
