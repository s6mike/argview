...
- `src/layouts/templates/pandoc-mapjs-main-html5.html`: Add site logo to main template.
  - `mapjs/public/mapjs-default-styles.css`: Make header container into flexbox, and set values for `h1` and `.site-logo` (both logo and wrapping anchor) so they wrap nicely.
  - Update doc example pages.
- [README.md](../README.md), [README-argview.md](../mapjs/docs/README-argview.md):
  - Add logo to title and move status bar there.
  - Update map images to link to live site version.
...
- `mapjs/webpack.config.js`: Fix SRI generation to be true for `NETLIFY == true` (online netlify server).
...
- `scripts/bash_aliases_mapjs.sh`: `testcafe_run()`: Print `target_url` so test log clearer.
...
- `netlify.toml`: Harden CSP headers further.
