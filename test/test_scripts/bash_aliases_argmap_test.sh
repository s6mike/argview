...
test_make_mapjs_dependencies() {
  ...
  make HTML_OPEN="false" KEEP_ORIGINAL=true TARGET_DOMAIN="$target_domain" "$file_html"
  rm "$file_json"
  make HTML_OPEN="$html_open" KEEP_ORIGINAL=true TARGET_DOMAIN="$target_domain" "$file_html"
  ls "$file_json"
}
