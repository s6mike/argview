...
# luarocks --lua-version=5.3 --lua-dir=$HOMEBREW_PREFIX/opt/lua@5.3 --tree "$install_dir/$dir_lua" make --only-deps "$rockspec_file" # This is for local/conda install only: YAML_LIBDIR="$PATH_ENVIRONMENT_GLOBAL/lib/"
# luarocks --lua-dir="$(brew --prefix)/opt/lua@5.3" --lua-version=5.3 init
# luarocks --tree lua_modules make --only-deps argmap-4.13.22-9.rockspec # YAML_LIBDIR="$PATH_ENVIRONMENT_GLOBAL/lib/"
luarocks --tree lua_modules --lua-dir="$(brew --prefix)/opt/lua@5.3" --lua-version=5.3 make --only-deps "$rockspec_file" # This is for local/conda install only: YAML_LIBDIR="$PATH_ENVIRONMENT_GLOBAL/lib/"

# echo "****luarocks customised****":
luarocks --tree lua_modules --lua-dir="$(brew --prefix)/opt/lua@5.3" --lua-version=5.3 list # This is for local/conda install only: YAML_LIBDIR="$PATH_ENVIRONMENT_GLOBAL/lib/"
