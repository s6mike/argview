...
`argmap2mup`, `argmap2tikz`, and `mup2argmap` require [penlight](https://github.com/stevedonovan/Penlight) (for command line option parsing), [lyaml](https://github.com/gvvaughan/lyaml) (for parsing `YAML`), and [json](https://github.com/rxi/json.lua) (for encoding `JSON`).
...
`argmap2mup` and `mup2argmap` also depend on the command line utility [`gdrive`](https://github.com/prasmussen/gdrive) for Google Drive integration. Follow the link for installation instructions.
...
Fixed some issues (potentially caused by different lua version), also had to remove #! lua directives from start of files due to vscode debugging issues. So have updated above instructions to include lua commands where appropriate.
